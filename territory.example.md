...
The walk reads this file at the start. It holds every behavior that is specific to your workspace, so the stage files stay portable. Everything below is a slot: a short note on what belongs there, plus a generic example you should overwrite. When you are done, every angle-bracket placeholder should be gone.
...
- Tier, declared in stage 1. Decide early how heavy the deliverable is.
  - Artifact-tier (big work): build the map as a committed document, with a diagram. State your format and location, for example `<your-docs-path>`.
  - Inline-tier (small walk): a decisions table in the reply, nothing committed.
